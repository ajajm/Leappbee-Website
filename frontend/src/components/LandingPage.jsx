...
const HeroBgSVG = () => (
  <svg xmlns="http://www.w3.org/2000/svg" width="1200" height="675" viewBox="0 0 1200 675" aria-hidden="true"
...
export default function LandingPage({ onAdminClick }) {
  ...
  useEffect(() => {
    (async function () {
      const ns = siteConfig ? siteConfig.calcomLink.split('/').pop() : "leappbee-discovery-call";
  ...
  const { brand, process: proc, stats, faq, footer } = CFG;

  const hero = siteConfig && siteConfig.calcomLink ? {
    ...CFG.hero,
    cta_primary: { ...CFG.hero.cta_primary, href: `https://cal.com/${siteConfig.calcomLink}?overlayCalendar=true` }
  } : CFG.hero;

  const contact = siteConfig ? {
    ...CFG.contact,
    discord: { ...CFG.contact.discord, url: siteConfig.discordUrl || '' },
    whatsapp: { ...CFG.contact.whatsapp, url: `https://wa.me/${siteConfig.whatsappNumber || ''}` },
    calcom_link: siteConfig.calcomLink || '',
  } : CFG.contact;

  const testimonials = siteConfig && siteConfig.testimonials ? {
    heading: CFG.testimonials.heading,
    row1: siteConfig.testimonials,
  } : CFG.testimonials;
  
  const faqItems = siteConfig && siteConfig.faqs ? siteConfig.faqs : faq.items;
  
  const footerColumns = siteConfig ? [
    footer.columns[0],
    { title: 'Services', links: (siteConfig.services || []).map(s => ({ label: s })) },
    { title: 'Connect', links: [
      { label: 'Discord Server', href: siteConfig.discordUrl || '' },
      { label: 'Book a Call', href: siteConfig.calcomLink ? `https://cal.com/${siteConfig.calcomLink}?overlayCalendar=true` : '' },
      { label: 'Email Us', href: footer.columns[2].links[2].href },
    ]}
  ] : footer.columns;
            ...
            <div className="contact-cards">
              ...
              <div className="ccard" style={{ textAlign: 'left' }}>
                ...
                <a href={contact.discord.url} target="_blank" rel="noreferrer"
                  className="btn-primary" style={{ background: '#5865F2', color: '#fff', display: 'flex', justifyContent: 'center', width: '100%', marginTop: 'auto' }}>
                  Join Discord Server
                </a>
              </div>
              ...
              <div className="ccard" style={{ textAlign: 'left' }}>
                ...
                <a href={contact.whatsapp.url} target="_blank" rel="noreferrer"
                  className="btn-primary" style={{ background: '#25D366', color: '#fff', display: 'flex', justifyContent: 'center', width: '100%', marginTop: 'auto' }}>
                  Message on WhatsApp
                </a>
              </div>
            </div>

            {/* Cal.com wrapper removed label */}
            <div className="calcom-container" style={{ width: '100%', marginTop: '32px' }}>
              <Cal 
                namespace={siteConfig ? siteConfig.calcomLink.split('/').pop() : "leappbee-discovery-call"}
                calLink={siteConfig ? siteConfig.calcomLink : "leappbee/leappbee-discovery-call"}
        ...
        <div className="wrap">
          <div className="footer-grid">
            <div style={{ paddingRight: '20px' }}>
              <div className="footer-logo-row" style={{ fontFamily: "'Inter Tight', sans-serif", fontSize: '24px', fontWeight: 'bold', textTransform: 'uppercase', marginBottom: '24px', letterSpacing: '0.05em' }}>
...
function VCard({ v, short = false }) {
  const { thumbnail, isYouTube, isDrive, embedUrl } = parseVideoUrl(v.url);
  const [playing, setPlaying] = useState(false);
  return (
    <div className="vcard" onClick={() => setPlaying(true)} style={{ cursor: playing ? 'default' : 'pointer' }}>
      <div className={`vcard-thumb ${short ? 'ar-9-16' : 'ar-16-9'}`} style={{ position: 'relative' }}>
        {!playing ? (
          <>
            <span className="vcard-tag">{v.category}</span>
            <img src={thumbnail} alt={v.title} className="vcard-img" loading="lazy" />
            ...
            <span className="vcard-badge">
              {isYouTube ? '▶ YouTube' : isDrive ? '📁 Drive' : 'Video'}
            </span>
          </>
        ) : (
          <iframe
